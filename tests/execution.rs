...
macro_rules! test_interpreter_and_jit {
    ...
    ($executable:expr, $mem:tt, $context_object:expr, $expected_result:expr $(,)?) => {
        ...
        #[cfg(all(feature = "jit", not(target_os = "windows"), target_arch = "riscv64"))]
        {
            ...
            // println!("{:?}",&compilation_result.as_ptr());
            let mut mem = $mem;
            let mem_region = MemoryRegion::new_writable(&mut mem, ebpf::MM_INPUT_START);
            create_vm!(
                vm,
            ...
            match compilation_result {
                ...
                Ok(()) => {
                    ...
                    println!("instruction_count_jit:{:?}", instruction_count_jit);
                    println!("context_object:{:?}", vm.context_object_pointer);
...
//     );
// }
//target/debug/deps/execution-ee621ea5f00d4c8d
