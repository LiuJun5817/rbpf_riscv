...
impl<'a, C: ContextObject> JitCompiler<'a, C> {
    ...
    fn emit_subroutines(&mut self) {
        ...
        self.emit_ins(X86Instruction::lea(OperandSize::S64, REGISTER_PTR_TO_VM, REGISTER_SCRATCH, Some(X86IndirectAccess::Offset(self.slot_in_vm(RuntimeEnvironmentSlot::ProgramResult)))));//lea是加载地址
        self.emit_ins(X86Instruction::load(OperandSize::S64, REGISTER_SCRATCH, REGISTER_MAP[0], X86IndirectAccess::Offset(8)));//load是加载该地址的值
        self.emit_ins(X86Instruction::return_near());
        ...
        self.emit_ins(X86Instruction::alu(OperandSize::S64, 0xc1, 5, REGISTER_SCRATCH, shift_amount as i64, None));//shift logical right
